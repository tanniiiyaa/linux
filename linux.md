...
Embedded OS: Designed for specific devices (e.g., IoT, routers).
...
System Libraries: Provide functionalities to the OS.
System Utilities: Tools for managing the system.
Shell: Interface for user commands.
Applications: Software running on top of the OS.
...
Enable line numbering and locate specific lines.
...
Open a file in Vim.
Use v to select a word or character sequence.
Use V to select a full line.
Use Ctrl + v to select a rectangular block of text.
...
Select the text.
Press c to clear and enter Insert Mode.
...
Root User: The administrator with full system access.
System Users: Used for system processes and services.
Regular Users: General users with restricted permissions.
Practical:
...
1: Link count (number of references to the file).
user: File owner.
group: Group assigned to the file.
2048: File size in bytes.
Dec 18 12:00: Last modification date and time.
file.txt: File name.
...
Change file permissions to rw- for the owner and no access for others:
$ chmod 600 file.txt
Verify permissions:
$ ls -l
...
Change directory permissions to allow group members to write:

$ chmod g+w /shared-directory
Restrict file access to the owner:

$ chmod 700 confidential.txt
Verify file type using file command:
...
Files and Hard Links: Each file starts with a link count of 1, representing its own name as a reference to its inode.
Directories: Directories typically have a link count greater than 2 due to . (current directory), .. (parent directory), and subdirectory entries.
Practical Example
...
Base Count: A directory always starts with at least two links: . (itself) and .. (parent).
Subdirectories: Each subdirectory increases the parent directory’s link count by 1.
Example
...
Hard Links: Creating additional names for the same file increases the link count.
Example
...
Edit sudoers File: Use the visudo command to safely edit the /etc/sudoers file.

Granting User Permissions: Add specific users or groups:

username ALL=(ALL) ALL
Restricting Commands: Limit commands a user can run:
...
Backups: Safeguard data by creating archives that can be stored securely.
Data Transfer: Simplify sharing by bundling multiple files into a single archive.
Organization: Group related files for better management.
...
Change the system date to simulate the execution time:

sudo date -s "23:59:50 28 dec 2024"
Verify the cron job is added by running:
...
Change the system date to simulate the execution time on Monday at 7:00 AM:
...
Search for all error logs in /var/log:
grep -r 'error' /var/log
Find and list all .log files modified in the last 2 days:
find /var/log -name "*.log" -mtime -2
Sort a file of names alphabetically and remove duplicates:
sort names.txt | uniq
...
Start a Process with Nice Value:
